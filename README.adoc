...
I skipped some details when they are not important for understanding the relationship between the keyboard and XKB (for example there is actually more than one mode for USB keyboards, which determine which usage code is sent).
...
* _Physical keyboard_: a set of physical keys (each with a position, a possibly complex shape, and the X keycode that the key sends when pressed)
* _Rectangular keyboard_: a physical keyboard whose keys all have a rectangular shape
